...
def write_metadata(directory: str, iteration: int) -> None:
    ...
    metadata = {"format_version": FORMAT_VERSION, "iteration": iteration}
...
def read_metadata(directory: str) -> dict:
    ...
    if not os.path.exists(metadata_path):
        raise FileNotFoundError(
            f"No '{METADATA_FILE}' in '{directory}'; not a complete alphazoo checkpoint."
        )
    ...
    if version != FORMAT_VERSION:
        raise ValueError(
            f"Unsupported checkpoint format_version {version!r} (expected {FORMAT_VERSION})."
        )
    return metadata

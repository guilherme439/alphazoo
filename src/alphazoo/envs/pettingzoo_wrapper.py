...
class PettingZooWrapper(IAlphazooGame):
    ...
    def encode_state(self) -> torch.Tensor:
        ...
        return EnvUtils.encode_observation(
            observation, self._observation_space, self._needs_transpose, self._obs_is_float32
        )
    ...
    def _compute_action_shape(self) -> tuple[int, ...]:
        return EnvUtils.action_shape(self.env.action_space(self.env.agent_selection))

    def _compute_state_shape(self) -> tuple[int, ...]:
        return tuple(self.encode_state().shape)

    def _compute_observation_space(self) -> Space:
        space = self.env.observation_space(self.env.agent_selection)
        if hasattr(space, 'spaces') and 'observation' in space.spaces:
            return space.spaces['observation']
        return space

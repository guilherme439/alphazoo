...
### Training Orchestrator (`alphazoo.py`)

`AlphaZoo` class manages the full training loop:
1. **Self-play phase** (`_run_selfplay`): delegates to the `SelfPlayCoordinator`, which drives the `Gamer` actors (each connected to the inference server, see Inference) and returns the games played this step; their trajectories are stored in a `ReplayBuffer`.
...
`_setup_actors` builds three coordinators and returns them to `train`, which starts them before the loop: the `SelfPlayCoordinator` (self-play), the `ReanalyseCoordinator` (reanalyse, a Ray actor, present only when reanalyse is enabled), and the `InferenceServer` (inference). Each exposes a uniform `alive()`; every step the driver calls `_alive()`, which polls all three so that a worker which died inside an unawaited long-running future surfaces its exception in the main loop instead of hanging silently.
...
Two execution modes, both implemented inside `SelfPlayCoordinator.play_step()`:
- **Sequential**: the coordinator submits `num_games_per_step` games to an internal `ray.util.ActorPool` of gamers and waits for all of them. Self-play and training never overlap.
- **Asynchronous**: gamers run `play_forever()`, pushing finished games onto an internal queue. Each step the coordinator polls `collect_completed_games()` until `update_delay` elapses, or - when `min_num_games` is set - until at least that many games have been collected (whichever comes first).
...
- `select_action_with_alphazero_mcts(env, model, search_config, obs_space_format, is_recurrent, recurrent_iterations)` (`utils.py`): top-level one-shot helper for external consumers. Wraps a PettingZoo env, builds a `LpcInferenceReplica` around `model` (in-process, no Ray), runs a fresh network-guided tree, returns the action.
...
Two built-in implementations of `IAlphazooGame` are provided. `AlphaZoo(env=...)` selects one through `EnvUtils.wrap` (`_internal_utils/env.py`): an `IAlphazooGame` is used as-is, a `gymnasium.Env` is wrapped in `GymWrapper`, and anything else is treated as a PettingZoo AECEnv and wrapped in `PettingZooWrapper`. Both wrappers share `EnvUtils.action_shape` and `EnvUtils.encode_observation` for their state/action conversion.
- `PettingZooWrapper` (`envs/pettingzoo_wrapper.py`): for PettingZoo AECEnv environments (two-player games).
- `GymWrapper` (`envs/gym_wrapper.py`): for single-agent Gymnasium environments with a `Discrete` action space. The agent is always player 1 (absolute value perspective); `terminal_value()` returns the reward of the terminating transition (suited to sparse, terminal-reward games); `Discrete` observations are one-hot encoded; `legal_actions_mask()` is all-ones.
...
### Reanalyse (`training/reanalyse/`)

When `learning.replay_buffer.reanalyse.enabled`, `_setup_actors` builds a `ReanalyseCoordinator` Ray actor that owns a pool of `num_workers` `Reanalyser` actors (`max_concurrency=2`, scheduled `SPREAD`) and an internal `queue.Queue` of pending work. The coordinator exposes `start()`, `enqueue(requests)`, `get_work()`, `collect_results()`, `alive()`, `get_pids()`, and `stop()`. On `start()` each worker runs a `run(coordinator)` loop: it pulls a batch of `ReanalyseRequest`s via `get_work()` (which blocks until at least one is queued, then drains a per-worker share of the queue), re-runs `Explorer.run_alphazero_mcts` on the game its `GameEncoder` decodes from each `request.entry.game_snapshot`, computes `policy = TrainingUtils.policy_from_root_visits(root, num_actions)` and `value = root.value()`, and buffers the resulting `ReanalyseResult` for the driver to collect.

At the start of each training step, `AlphaZoo._run_reanalyse`:
1. Collects any finished results from the coordinator (`collect_results()`) and applies each via `ReplayBuffer.apply_reanalyse_result`, decrementing the tracked `_reanalyse_backlog`.
2. If `positions_per_step > 0` and `ReplayBuffer.fill_ratio() >= min_buffer_fill_ratio`, calls `ReplayBuffer.pop_oldest(positions_per_step)`, wraps each popped entry in a `ReanalyseRequest`, and hands the batch to the coordinator via `enqueue(requests)` (adding to `_reanalyse_backlog`).
...
`PettingZooWrapper.serialize` / `deserialize` (both `@staticmethod`) walk the env-wrapper chain layer-by-layer, dumping each layer's `(type, attrs)` plus the outer wrapper class to a plain dict, bypassing `EzPickle`'s `__getstate__` (which would otherwise discard the env's live runtime state on round-trip) so a snapshot resumes from the exact same position. In-process cloning (`__deepcopy__`) rebuilds every layer via `object.__new__`. Cross-process loading (`__setstate__`) rebuilds the base env through its real `__init__` from the stored EzPickle args - re-establishing any process-global state its `__init__` sets up so the env is valid in the receiving process - and the surrounding wrapper layers via `object.__new__`, then overlays the saved runtime attributes. `GameEncoder` (`training/game_encoder.py`) wraps these: `encode(game)` runs `serialize(game)`, optionally zlib-compresses it (selected by `learning.replay_buffer.reanalyse.compress_games`), and prefixes a one-byte header recording whether the payload is compressed; `decode(data)` reads that header, decompresses if needed, and calls `deserialize`. The driver builds one `GameEncoder` when reanalyse is enabled and hands it to the gamers and reanalysers. Games are encoded at capture in `GameRecord.store_step` and decoded in `Reanalyser._process`, so buffer snapshots travel across Ray as plain `bytes`.

`TrainingUtils.policy_from_root_visits` (`_internal_utils/training.py`) builds the visit-count-derived policy distribution from a search root; used by both `GameRecord.store_visit_counts` and `Reanalyser._process`.

### Distributed Architecture

Driver-side coordinators and the Ray actors they own:
- `SelfPlayCoordinator` (`training/selfplay/selfplay_coordinator.py`): a plain driver-side object that constructs and drives the `Gamer` actors. Exposes `start()`, `play_step() -> list[GameRecord]`, `collect_completed_games()`, `alive()`, `stop()`, `get_pids()`, and `get_metrics()`. Holds the sequential `ActorPool` or the async `play_forever()` futures depending on `running_mode`.
- `Gamer` (`training/selfplay/gamer.py`): each Gamer is a Ray actor that plays games using MCTS. Holds one or more `IInferenceClient` handles for neural network evaluations. `play_games(n) -> list[GameRecord]` returns records directly (used in sequential mode via `ActorPool`); `play_forever()` pushes records to an internal queue drained via `get_completed_games()` (used in async mode); `stop()` halts the async loop. Takes an optional `GameEncoder` which it passes to each `GameRecord` to perform snapshot capture.
- `ReanalyseCoordinator` (`training/reanalyse/reanalyse_coordinator.py`): a Ray actor that owns the `Reanalyser` pool and the pending-work queue (see Reanalyse). Created with `max_concurrency = num_workers + 2`.
- `Reanalyser` (`training/reanalyse/reanalyser.py`): each Reanalyser is a Ray actor (`max_concurrency=2`, scheduled `SPREAD`) running the coordinator-driven `run(coordinator)` loop and buffering `ReanalyseResult`s for `get_results()`. Holds its own `IInferenceClient` instances - provisioned alongside the gamer clients when the inference server is created.
- `InferenceServer` (`inference/server.py`): driver-side orchestrator over the inference replicas (see Inference).
- `ReplayBuffer`: shared training data store.

### Inference (`inference/`)

Inference is organised behind two interfaces and a base server. `IInferenceClient` is what consumers hold to evaluate a state. `IInferenceReplica` is one inference worker: it holds a model copy, mints the clients bound to it, and runs its own batching and cache. The base `InferenceServer` (`inference/server.py`) is the front the driver holds: it owns one or more replicas, hands out their clients as one flat list, and fans lifecycle calls (`start` / `stop` / `publish_model`) out to every replica, while one typed subclass per backend builds the replicas it needs. Inference never flows through the server - each client talks to its own replica directly.

Replicas come in three deployment modes, picked based on where the consumer runs relative to the model:

- **`lpc/`** — Local Procedure Call. `LpcInferenceReplica` holds a `ModelHost` and serves requests synchronously to in-process `LpcInferenceClient` instances. No Ray, no shared memory. Single-process only, since its clients hold a direct reference to it. Used for one-shot external entry points like `select_action_with_alphazero_mcts`.
- **`ipc/`** — Inter-Process Communication. `IpcInferenceReplica` is a Ray actor on the same machine as its clients. It holds a `ModelHost` and serves inference to `IpcInferenceClient`s in other processes. Transport is owned by `InferenceSlot`: named shared memory for zero-copy tensor passing and named FIFO pipes for ready/done signalling. Inference runs as a three-stage pipeline so I/O overlaps GPU compute: a collector thread waits on every slot's ready fd via `epoll` and answers cache hits directly, a GPU thread drains the misses into one stacked forward pass, and a writer thread returns results to their slots; the stages are connected by queues and stopped via a sentinel hand-off. Used by `AlphaZoo` for training-time self-play on a single machine.
- **`rpc/`** - Remote Procedure Call. `RpcInferenceReplica` is a Ray actor reachable from clients on any node. It holds a `ModelHost` and serves inference to `RpcInferenceClient`s through Ray method calls. A cache hit is answered inline on the calling thread; misses are enqueued and a single batcher thread drains them into one stacked forward pass, returning each result through a per-request event. Each client call occupies one actor-concurrency thread, so the actor is created with `max_concurrency = num_clients + reserve`. Used by `AlphaZoo` for training-time self-play across multiple nodes.

For the ipc and rpc actor replicas the orchestrator holds a `RemoteInferenceReplica` (`inference/remote_replica.py`): an in-process handle that forwards each `IInferenceReplica` call to the Ray actor and resolves it, so a local (lpc) replica and a remote one drive through the same synchronous surface.

`AlphaZoo` builds the training-time inference stack by constructing a typed server. `InferenceUtils.resolve_inference_backend` (`_internal_utils/inference.py`) maps `running.inference_backend` (`auto` / `ipc` / `rpc`) to a transport - `auto` selects `rpc` when Ray reports more than one live node and `ipc` otherwise, starting a local single-node Ray if none is running - and the driver constructs the matching `IpcInferenceServer` or `RpcInferenceServer`; the standalone MCTS util constructs `LpcInferenceServer`. `running.inference_gpus` sets the number of replicas, one per GPU: `None` uses every discrete GPU (integrated GPUs are skipped, since they cannot run the compute kernels), and an explicit value is capped at that count and at the worker count. Each self-play worker (a gamer or reanalyser, with all its search threads) is assigned whole to one replica, so its clients share that replica's cache and batch queue.

The interfaces are tiny: clients expose a single `inference(state)` returning `(policy_logits, value)`; replicas expose `get_clients()` and `publish_model(state_dict)` alongside lifecycle and metrics hooks, and the orchestrator presents the same `get_clients()` / `publish_model()` surface over the whole replica set. Whether the underlying model is recurrent and how many recurrent iterations to run are replica-side settings; callers don't need to know. Consumers (e.g. `Gamer`, `Explorer`) depend only on `IInferenceClient`.

Weight updates on the `IpcInferenceReplica` and `RpcInferenceReplica` are protected by a read-write lock: the inference thread takes the read lock around forward + cache put for each batch; `publish_model()` takes the write lock to swap weights and invalidate the cache atomically.
...
- **`MetricsRecorder`** (`recorder.py`): lightweight, thread-safe recorder used in each component (Gamer, IpcInferenceReplica, NetworkTrainer, AlphaZoo). Records metrics with typed methods: `scalar`, `mean`, `counter`, `lifetime_counter`, `lifetime_scalar`. `drain()` returns a snapshot and resets per-step metrics (lifetime metrics persist).
